...
def list_and_list_to_list(lsta, lstb):
    ...
    """
    new_list = []
    new_list.append(lsta)
    new_list.append(lstb)
    return new_list # implement me
...
def list_from_range4(n, m):
    ...
    """
    return list(range(n + 1, m + 1)) # implement me
...
def concat_list_indexwise(lst1, lst2):
    ...
    """
    # result = []
    # for i in range(0, min(len(lst1), len(lst2))):  #for loop method
    #     result.append(lst1[i] + lst2[i])

    result = [lst1[i] + lst2[i  ] for i in range(min(len(lst1), len(lst2)))] ## list comprehension
    return result
...
def remove_item_from(lst, aaa):
    ...
    """
    result = [i for i in lst if i != aaa]
    return result
